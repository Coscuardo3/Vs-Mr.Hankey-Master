...
class PlayState extends MusicBeatState
{
	...
	public var kennyGroup:FlxTypedGroup<Character>;
	...
	var shit:FlxSprite;
	var knScared:FlxSprite;
	var audience:FlxSprite;

	var kenny:Character;
	var kennycam:FlxObject = new FlxObject(200, 350, 1, 1);

	var iskenny:Bool = false;
	var isbf:Bool = true;
	var isdad:Bool = true;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'christmas':
			{
					defaultCamZoom = 0.7;
					curStage = 'christmas';
					var skyChristmas:FlxSprite = new FlxSprite(-302, -130).loadGraphic(Paths.image('weeb/skyChristmas'));
					skyChristmas.setGraphicSize(Std.int(skyChristmas.width * 1.9));
					skyChristmas.antialiasing = ClientPrefs.globalAntialiasing;
					skyChristmas.scrollFactor.set(0.95, 0.95);
					skyChristmas.active = false;
					add(skyChristmas);

					var treesChristmas:FlxSprite = new FlxSprite(-1270, -600).loadGraphic(Paths.image('weeb/treesChristmas'));
					treesChristmas.setGraphicSize(Std.int(treesChristmas.width * 1.95));
					treesChristmas.updateHitbox();
					treesChristmas.antialiasing = ClientPrefs.globalAntialiasing;
					treesChristmas.scrollFactor.set(0.9, 0.9);
					treesChristmas.active = false;
					add(treesChristmas);

					sceneChristmas = new FlxSprite(0, -700);
					sceneChristmas.frames = Paths.getSparrowAtlas('weeb/sceneChristmas');
					sceneChristmas.setGraphicSize(Std.int(sceneChristmas.width * 1));
					sceneChristmas.animation.addByPrefix('idle', "lightsChristmas", 24);
					sceneChristmas.animation.play('idle');
					sceneChristmas.screenCenter(X);
					sceneChristmas.antialiasing = ClientPrefs.globalAntialiasing;
					sceneChristmas.scrollFactor.set(0.947, 0.947);
					sceneChristmas.updateHitbox();
					sceneChristmas.alpha = 1;
					add(sceneChristmas);

					audience = new FlxSprite(-1700, 620);
					audience.frames = Paths.getSparrowAtlas('weeb/Audience');
					audience.setGraphicSize(Std.int(audience.width * 2.7));
					audience.animation.addByPrefix('idle', "audiencia", 24);
					audience.animation.play('idle');
					audience.antialiasing = ClientPrefs.globalAntialiasing;
					audience.scrollFactor.set(0.95, 0.95);
					audience.updateHitbox();
					audience.alpha = 0;

					shit = new FlxSprite(0, 0);
					shit.frames = Paths.getSparrowAtlas('weeb/shit');
					shit.setGraphicSize(Std.int(shit.width * 1));
					shit.animation.addByPrefix('stop', "stop", 24);
					shit.animation.addByPrefix('idle', "caca", 24);
					shit.animation.play('stop');
					shit.screenCenter(X);
					shit.screenCenter(Y);
					shit.antialiasing = true;
					shit.scrollFactor.set(0.95, 0.95);
					shit.updateHitbox();
					shit.alpha = 1;
            }
			case 'christmas2':
			{
					...
					var skyChristmas:FlxSprite = new FlxSprite(-302, -130).loadGraphic(Paths.image('weeb/skyChristmas'));
					skyChristmas.setGraphicSize(Std.int(skyChristmas.width * 1.9));
					skyChristmas.antialiasing = true;
					skyChristmas.scrollFactor.set(0.95, 0.95);
					skyChristmas.active = false;
					add(skyChristmas);
					...
					knScared = new FlxSprite(100, 370);
					knScared.frames = Paths.getSparrowAtlas('weeb/KNHoliScared');
					knScared.setGraphicSize(Std.int(knScared.width * 1));
					knScared.animation.addByPrefix('idle', "KN idle", 24);
					knScared.animation.play('idle');
					knScared.antialiasing = ClientPrefs.globalAntialiasing;
					knScared.scrollFactor.set(0.947, 0.947);
					knScared.updateHitbox();
					knScared.alpha = 1;
		...
		kenny = new Character(100, 460, 'kennychristmas');
		...
		timeBar.createFilledBar(0xFF000000, 0xFF0C9E02); 
		...
		if (FlxG.save.data.snow && curSong == 'brown-christmas')
		{
			snowfallSpr.cameras = [camHUD];
		}

		if (FlxG.save.data.snow && curSong == 'poopstorm')
		{
			add(snowfallSpr);
			snowfallSpr.cameras = [camHUD];
		}
	...
	function doStopSign(sign:Int = 0, fuck:Bool = false)
	{
		...
		daSign.frames = Paths.getSparrowAtlas('characters/Sign_Post_Mechanic');
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(tmr.loopsLeft % 2 == 0) {
					...
					if (kenny.animation.curAnim != null && !kenny.animation.curAnim.name.startsWith('sing') && !kenny.specialAnim)
					{
						kenny.dance();
					}
				}
				...
				else if(kenny.danceIdle && kenny.animation.curAnim != null && !kenny.specialAnim && !kenny.curCharacter.startsWith('gf') && !kenny.animation.curAnim.name.startsWith("sing"))
				{
					kenny.dance();
				}
	...
	var rotInd:Int = 0;  //this code is from Vs Shaggy <3
	var wb_eX:Float = 0;
	var wb_eY:Float = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'shitStorm':
				if (dad.curCharacter == 'mrHankeyWizard')
				{
					rotInd ++;
					var rot = rotInd / 6;

					dad.x = DAD_X + Math.cos(rot / 7) * 60 + wb_eX;
					dad.y = DAD_Y + Math.cos(rot / 10) * 40 + wb_eY;
				}
		}

		//if (curSong == 'brown-christmas' && curStep != stepOfLast) 
		//{
			//switch(curStep)
			//{
				//case 28:
		...
		{
			if (iskenny == true)
				camFollow.set(kenny.getMidpoint().x + 150, kenny.getMidpoint().y - 250);
			else
		...
		if (curSong == 'brown-christmas' && curStep != stepOfLast) 
		{
			var blackScreen:FlxSprite = new FlxSprite().makeGraphic(Std.int(FlxG.width * 5), Std.int(FlxG.height * 5), FlxColor.BLACK);
			...
			switch(curStep)
			{
				...
				case 912:
					...
					FlxTween.tween(FlxG.camera, {zoom: 1.1}, 6.0, {
						ease: FlxEase.linear,
						onComplete: function(twn:FlxTween)
						{
							defaultCamZoom = 0.7;
					...
					FlxTween.tween(blackScreen, {alpha: 0.2}, 8.0, {
						ease: FlxEase.linear,
						onComplete: function(twn:FlxTween) {
							remove(blackScreen);
							...
							add(blackScreen);
							audience.alpha = 0;
							new FlxTimer().start(22.7, function(tmr:FlxTimer)
							{
								remove(blackScreen);
							});
						}
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 2 && dad.animOffsets.exists('hey')) {
						...
						if (iskenny)
							{
								kenny.playAnim(animToPlay + altAnim, true);
								kenny.holdTimer = 0;
							}
						if (isdad)
							{
	...
	override function stepHit()
	{
		...
		if (curStage == 'sos' && curStep != stepOfLast)
		{
			switch(curStep)
			{
				case 3:
